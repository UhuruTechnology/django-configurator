...
from dj_configurator.importer import ConfigurationFinder

# ROOT_DIR = os.path.dirname(os.path.dirname(__file__))
# TEST_PROJECT_DIR = os.path.join(ROOT_DIR, 'sample_project')
...
class MainTests(TestCase):
    ...
    @patch.dict(os.environ, clear=True, DJANGO_SETTINGS_MODULE="tests.settings.main")
    ...
    def test_repr(self):
        ...
        self.assertEqual(repr(Test), "<Configuration 'tests.settings.main.Test'>")
    ...
    def test_initialization(self):
        ...
        self.assertEqual(
            repr(finder), "<ConfigurationFinder for 'tests.settings.main.Test'>"
        )
    ...
    def test_initialization_inheritance(self):
        finder = ConfigurationFinder()
        self.assertEqual(finder.module, "tests.settings.inheritance")
    ...
    def test_configuration_argument_in_cli(self):
        ...
        proc = subprocess.Popen(
            ["django-cadmin", "test", "--help"], stdout=subprocess.PIPE
        )
        self.assertIn("--configuration", proc.communicate()[0].decode("utf-8"))
        proc = subprocess.Popen(
            ["django-cadmin", "runserver", "--help"], stdout=subprocess.PIPE
        )
        self.assertIn("--configuration", proc.communicate()[0].decode("utf-8"))
    ...
    def test_configuration_argument_in_runypy_cli(self):
        ...
        """
        proc = subprocess.Popen(
            [sys.executable, "-m", "dj_configurator", "test", "--help"],
            stdout=subprocess.PIPE,
        )
        self.assertIn("--configuration", proc.communicate()[0].decode("utf-8"))
        proc = subprocess.Popen(
            [sys.executable, "-m", "dj_configurator", "runserver", "--help"],
            stdout=subprocess.PIPE,
        )
        self.assertIn("--configuration", proc.communicate()[0].decode("utf-8"))
    ...
    def test_django_setup_only_called_once(self):
        proc = subprocess.Popen(
            [sys.executable, os.path.join(os.path.dirname(__file__), "setup_test.py")],
            stdout=subprocess.PIPE,
        )
    ...
    def test_utils_reraise(self):
        from dj_configurator.utils import reraise
