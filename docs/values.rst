...
.. module:: dj_configurator.values
   :synopsis: Optional value classes for high-level validation and behavior.

.. versionadded:: 0.4

django-configurator allows you to optionally reduce the amount of validation
...
Luckily django-configurator' :class:`~Value` subclasses have the ability
...
default) to tell django-configurator to look for an environment variable with
...
django-configurator will try to read the ``DJANGO_ROOT_URLCONF`` environment
...
django-configurator allows you to use the ``environ_name`` parameter of the
...
.. code-block:: pycon
    ...
    >>> type(values.Value([]))
    <class 'dj_configurator.values.Value'>
...
.. class:: Value(default, [environ=True, environ_name=None, environ_prefix='DJANGO', environ_required=False])
   ...
   .. method:: setup(name)
      ...
      to return a value django-configurator is supposed to use when loading
